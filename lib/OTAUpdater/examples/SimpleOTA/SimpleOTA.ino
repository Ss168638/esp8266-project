...
OTAUpdater updater;
void setup(){Serial.begin(115200);WiFi.begin(ssid,pass);while(WiFi.status()!=WL_CONNECTED){delay(300);} updater.setUrls(VERSION_URL,FIRMWARE_URL);updater.setCurrentVersion("1.0.3");updater.beginClient();updater.checkAndUpdate();}
void loop(){}
